/* Hokkaido MOGU Map v5: Hokkaido-first precise pins + Google/UW links */
(() => {
  ...
  window.openGoogleListing = function(id){
    const s=shops.find(x=>x.id===id);
    if(s && hasGoogleListing(s)) window.open(s.google_listing_url,'_blank','noopener');
  };
  window.openUwPage = function(id){
    const s=shops.find(x=>x.id===id),u=communityLink(s);
  ...
  window.openDetail = function(id){
    ...
    const precise=isPrecise(s),google=hasGoogleListing(s),uw=communityLink(s);
    let routeHtml=`<h3>地図・店舗情報・コミュニティ</h3>${s.stamp_rally_enabled?`<div class="rallyDetailBanner">🎫 スタンプラリー参加スポット<br><span>${kindLabel(s)}</span></div>`:''}<div class="actions">`;
    ...
    if(!google&&!uw)routeHtml+='<p class="muted">Google店舗ページ・UWコミュページは現在未登録です。</p>';
    const firstHr=detail.querySelector('hr'); if(firstHr)firstHr.insertAdjacentHTML('beforebegin',routeHtml);else detail.insertAdjacentHTML('beforeend',routeHtml);
  };

  function boot(){if(window.map&&window.shops&&Array.isArray(shops)){applyJapanBounds();renderShops();}else setTimeout(boot,120)}
  boot();
})();
