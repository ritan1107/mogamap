(() => {
  ...
  async function loadMyPage(){
    ...
    try{
      ...
      myPageProfile=Array.isArray(p)?p[0]:p;myPageHistory=Array.isArray(h)?h:[];publicProfile=(Array.isArray(pub)?pub[0]:pub)||{ranking_public:false};
  ...
  function renderMyPage(){
    ...
    a.innerHTML=`<div class="myHero"><div class="myAvatar">😊</div><div><span class="myHello">こんにちは</span><h2>${esc(p.nickname)}さん</h2></div></div>
      ...
      <div class="mySection promoSettings"><h3>🏆 ランキング・宣伝プロフィール</h3><p class="muted">公開をONにした人だけランキングに表示されます。OFFなら名前も宣伝内容も公開されません。</p><label class="promoToggle"><input id="rankingPublic" type="checkbox" ${q.ranking_public?'checked':''}><span><b>ランキングに自分を表示する</b><small>スタンプ上位20名に入ると表示されます</small></span></label><label class="field full"><span>ランキング表示名</span><input id="publicDisplayName" maxlength="40" value="${esc(q.public_display_name||p.nickname||'')}" placeholder="例：りーたん"></label><label class="field full"><span>宣伝タイトル</span><input id="promoTitle" maxlength="60" value="${esc(q.promo_title||'')}" placeholder="例：Web制作・LINEスタンプ作ります"></label><label class="field full"><span>ひとこと・紹介文</span><textarea id="promoText" maxlength="180" placeholder="お店・サービス・活動など、伝えたいことを書けます">${esc(q.promo_text||'')}</textarea></label><label class="field full"><span>詳しく見るURL</span><input id="promoUrl" type="url" value="${esc(q.promo_url||'')}" placeholder="https://..."></label><button class="btn promoSave" onclick="saveMyPublicProfile()">この公開設定を保存</button><p id="promoSaveMsg" class="muted"></p></div>
      <div class="mySection"><h3>✅ 獲得したスタンプ</h3>${history}</div>
      <div class="mySection myGuide"><h3>このマイページでできること</h3><p>🪪 参加者IDを確認・コピー<br>🎫 スタンプ数を確認<br>🏆 ランキング公開を自分でON/OFF<br>📣 お店・サービス・活動を紹介<br>✅ 獲得した店舗を確認<br>📦 遠方・オンライン購入のスタンプも確認</p></div>`;
  }
