(() => {
  ...
  window.loadPublicRanking=async function(){
    ...
    try{
      ...
      box.innerHTML=`<div class="rankingHead"><div><span>みんなの挑戦</span><h3>🏆 スタンプランキング TOP20</h3></div><small>公開ONの参加者のみ</small></div><div class="rankingList">${rows.map(r=>{const medal=r.rank_no===1?'🥇':r.rank_no===2?'🥈':r.rank_no===3?'🥉':'🏅';const promo=(r.promo_title||r.promo_text)?`<div class="rankPromo">${r.promo_title?`<b>${esc(r.promo_title)}</b>`:''}${r.promo_text?`<p>${esc(r.promo_text)}</p>`:''}${r.promo_url?`<a href="${esc(r.promo_url)}" target="_blank" rel="noopener" onclick="event.stopPropagation()">詳しく見る →</a>`:''}</div>`:'';return `<article class="rankCard rank${r.rank_no<=3?'Top':''}"><div class="rankNo">${medal}<strong>${r.rank_no}</strong></div><div class="rankMain"><div class="rankName">${esc(r.display_name||'参加者')}</div><div class="rankCount">🎫 ${r.stamp_count||0} スタンプ</div>${promo}</div></article>`}).join('')}</div><p class="rankingFoot">自分も表示したい人は「マイページ → ランキング・宣伝プロフィール」から公開できます。</p>`;
